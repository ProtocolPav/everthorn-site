...
import { useServerInfo, useServerStatus, serverInfoQueryOptions, serverStatusQueryOptions } from "@/hooks/use-info";
import { usePlayers } from "@/hooks/use-players";

const cpuChartConfig = {
    cpu: { label: "CPU Usage", color: "#22c55e" },
} satisfies ChartConfig;

const ramChartConfig = {
    ram: { label: "RAM Usage", color: "#f59e42" },
} satisfies ChartConfig;
...
export default function ServerOverview() {
    ...
    const { data: players, isLoading: playersLoading } = usePlayers("611008530077712395");
        ...
        <div className="grid grid-cols-3 lg:grid-cols-5 gap-2">
                ...
                <div className="relative z-10 p-3 h-full w-full flex items-center justify-between">
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                        ...
                        <div className="text-lg font-bold leading-none truncate">
                            {infoLoading ? <Skeleton className="h-5 w-32" /> : "Everthorn"}
                        </div>
                    ...
                    <div className="flex items-center gap-2">
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent side="bottom" align="end">
                                <DropdownMenuItem
                                    onClick={() => handleActionWithPolling("/amethyst/controls/restart", "started")}
                                >
                                    <RotateCw className="w-4 h-4 mr-2" /> Restart
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                    onClick={() => handleActionWithPolling("/amethyst/controls/kill", "stopped")}
                                >
                ...
                <div className="absolute inset-0 opacity-50">
                    <ChartContainer config={cpuChartConfig} className="h-full w-full">
                        <AreaChart data={cpuData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                            <Area dataKey="cpu" type="natural" fill={cpuChartConfig.cpu.color} fillOpacity={0.4} stroke={cpuChartConfig.cpu.color} strokeWidth={2} />
                        </AreaChart>
                    </ChartContainer>
                </div>
                <div className="relative z-10 p-3 h-full flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">CPU</p>
                        <div className="text-lg font-bold leading-none">{Math.round(cpuData.at(-1)?.cpu ?? 0)}%</div>
                ...
                <div className="absolute inset-0 opacity-50">
                    <ChartContainer config={ramChartConfig} className="h-full w-full">
                        <AreaChart data={ramData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                            <Area dataKey="ram" type="natural" fill={ramChartConfig.ram.color} fillOpacity={0.4} stroke={ramChartConfig.ram.color} strokeWidth={2} />
                        </AreaChart>
                    </ChartContainer>
                </div>
                <div className="relative z-10 p-3 h-full flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">RAM</p>
                        <div className="hidden lg:block text-lg font-bold leading-none">{usedRAM} GB / {totalRAM} GB</div>
                        <div className="lg:hidden text-lg font-bold leading-none">{usedRAM} GB</div>
                        <p className="text-xs text-muted-foreground mt-0.5">Usage</p>
                    </div>
            ...
            <Card className="col-span-2 lg:col-span-1 border-0 bg-gradient-to-br from-purple-50 to-violet-50 dark:from-purple-950/20 dark:to-violet-950/20 h-20">
                <div className="p-3 h-full flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Players</p>
                        <div className="text-lg font-bold leading-none">
                            {playersLoading ? <Skeleton className="h-5 w-8" /> : players?.length || "0"}
                        </div>
