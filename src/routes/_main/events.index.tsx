...
import {CalendarIcon, ClockIcon, ConfettiIcon, GlobeIcon, SparkleIcon, TrophyIcon, UsersIcon} from "@phosphor-icons/react"
import {cn} from "@/lib/utils"
import {events, EventData} from "@/config/events.ts"
...
function FeaturedEventCard({ event }: { event: EventData }) {
    ...
    const wrapperProps = !isClickable ? { className: "block" } : { to: `/events/${event.slug}`, className: "block" };
...
function EventCard({ event }: { event: EventData }) {
    ...
    const wrapperProps = isUpcoming ? { className: "block h-full" } : { to: `/events/${event.slug}`, className: "block h-full" };
